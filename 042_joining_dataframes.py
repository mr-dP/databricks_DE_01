...
countries.join(regions, countries["region_id"] == regions["id"], "right").select(
    countries["name"], regions["name"], countries["population"]
).display()

# COMMAND ----------

countries.join(regions, countries["region_id"] == regions["id"], "inner").select(
    countries["name"].alias("country_name"),
    regions["name"].alias("region_name"),
    countries["population"],
).orderBy(countries["population"], ascending=False).display()
